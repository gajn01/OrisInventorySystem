<?php
    ...
    $sql=("SELECT * FROM tbl_history WHERE account_id = $account_id AND (product_category LIKE '$search%' OR product_name LIKE '$search%' OR full_name LIKE '$search%' )  LIMIT $limit OFFSET $page ");
    $result = mysqli_query($db, $sql);
    $fetch = mysqli_fetch_all ($result, MYSQLI_ASSOC);
    if($fetch){
        $form_data['success'] = true;
