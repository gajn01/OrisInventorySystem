<?php
    ...
    $sql=("SELECT * FROM tbl_inventory WHERE product_category = $category_id AND (product_name LIKE '$search%' OR product_description LIKE '$search%'  OR product_location LIKE '$search%')  LIMIT $limit OFFSET $page ");
    $result = mysqli_query($db, $sql);
    $fetch = mysqli_fetch_all ($result, MYSQLI_ASSOC);
    if($fetch){
        $form_data['success'] = true;
